...
- **自动中和强制分页 CSS**：自动覆盖并重置 HTML 中的 `page-break-*` 与 `break-*` 等打印分页指令，防止 Chromium 打印引擎强行把内容切页导致正文静默丢失。
...
4. **规则中和**：动态注入覆盖样式，强制中和所有 `page-break` 与 `break` 分页指令。
5. **尺寸适配**：实时提取当前文档在渲染后的 `scrollWidth` 与 `scrollHeight`。
6. **视口锁定与捕获**：强行改变 Playwright 内部视口（Viewport）大小匹配目标分辨率，杜绝滚动条，剔除浏览器边距后写入字节流。
